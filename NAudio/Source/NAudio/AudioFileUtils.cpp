...
namespace NAudio {
#ifdef __APPLE__
	void
	checkCAError(OSStatus error, const char* operation) {
		if(error == noErr) {
			return;
		}
	...
	SampleTable
	loadAudioFile(std::string path, int numChannels) {
	...
	SampleTable
	loadAudioFile(std::string path, int numChannels) {
		LOG(NLOG_ERROR, "LoadAudioFile is currently only implemented for Apple platforms.");
		return(NULL);
	}
#endif
}
