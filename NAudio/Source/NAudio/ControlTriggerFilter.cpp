...
namespace NAudio {
	namespace NAudio_DSP {
		...
		void
		ControlTriggerFilter_::computeOutput(const SynthesisContext_& context) {
		...
		void
		ControlTriggerFilter_::sequence(std::string seqArg) {
			sequence_.clear();
		...
		void
		ControlTriggerFilter_::trigger(ControlGenerator trigger) {
			trigger_ = trigger;
		}

		void
		ControlTriggerFilter_::setLoop(ControlGenerator loop) {
			loop_ = loop;
		}
	}
}